...
{
	switch (NTSTATUS status = NtQuerySecurityObject(Handle, PROCESS_TRUST_LABEL_SECURITY_INFORMATION|
	...
	case STATUS_INVALID_PARAMETER_2:
		return NtQuerySecurityObject(Handle, 
			OWNER_SECURITY_INFORMATION|DACL_SECURITY_INFORMATION|LABEL_SECURITY_INFORMATION, 
			SecurityDescriptor, Length, LengthNeeded);
...
void DumpObjectSecurity(WLog& log, LSA_LOOKUP_HANDLE PolicyHandle, HANDLE hObject)
{
	...
	do 
	{
		...
		if (0 <= (status = QuerySecurityObject(hObject, psd, cb, &rcb)))
		{
			//log(L)
			PACL Acl;
